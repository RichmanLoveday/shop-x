...
use App\Services\Contracts\Vendor\ProductDigitalFileServiceInterface;
...
class ProductDigitalFileService extends BaseService implements ProductDigitalFileServiceInterface
{
    ...
    public function handleChunkUpload(int $productId, int $storeId, User $user, ProductType|string $type, array $data): array
    {
        $product = $this->productRepo->getProduct($productId, $type, $storeId);
    ...
    public function getDigitalFile(int $productId, int $storeId, int $fileId, ProductType|string $type = ProductType::PHYSICAL): ProductFile
    {
        $product = $this->productRepo->getProduct($productId, $type, $storeId);
        return $this->productRepo->findDigitalFile($fileId, $productId);
    }

    public function deleteDigitalFile(int $productId, int $storeId, int $fileId): bool
    {
    ...
    private function mergeChunks(string $uuid, $file, User $user, int $totalChunks, int $productId): array
    {
